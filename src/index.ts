...
async function executeStep(
  ...
  - Use the **read file** tool to inspect the current contents.
  - Use the **anchor edit** tool to overwrite the file with the new content.
...
const cookedTask2 = `Insert the following code starting at line 6 in the file located at: ${path.join(getProjectAbsolutePath(), '/src/server.js')}
...
// `;
      ...
      const input = {
        messages: [new HumanMessage(dTask)],
      };
...
async function executeCodeGen(
  ...
  console.log("File content to generate code from:", fileContent);
